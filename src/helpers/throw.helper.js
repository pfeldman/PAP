...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push( () => {
  _gsScope._gsDefine("plugins.ThrowPropsPlugin", ["plugins.TweenPlugin", "TweenLite", "easing.Ease", "utils.VelocityTracker"], (TweenPlugin, TweenLite, Ease, VelocityTracker) => {
    const ThrowPropsPlugin = function() {
      TweenPlugin.call(this, "throwProps");
      this._overwriteProps.length = 0;
    };

    const _max = 999999999999999;
    const _min = 0.0000000001;
    const _globals = _gsScope._gsDefine.globals;

    let _recordEndMode = false;

    const _transforms = {x:1,y:1,z:2,scale:1,scaleX:1,scaleY:1,rotation:1,rotationZ:1,rotationX:2,rotationY:2,skewX:1,skewY:1,xPercent:1,yPercent:1};

    const _getClosest = (n, values, max, min, radius) => {
      let i = values.length;
      let closest = 0;
      let absDif = _max;
      let val;
    ...
    const _parseEnd = (curProp, end, max, min, name, radius) => {
      if (curProp.end === "auto") {
        return curProp;
      }
      const endVar = curProp.end;
      let adjustedEnd;
      let p;
    ...
    const _extend = (decoratee, extras, exclude) => {
    ...
    const _calculateChange = ThrowPropsPlugin.calculateChange = (velocity, ease, duration, checkpoint) => {
      if (checkpoint == null) {
        checkpoint = 0.05;
      }
      const e = (ease instanceof Ease) ? ease : (!ease) ? TweenLite.defaultEase : new Ease(ease);
      return (duration * checkpoint * velocity) / e.getRatio(checkpoint);
    };

    const _calculateDuration = ThrowPropsPlugin.calculateDuration = (start, end, velocity, ease, checkpoint) => {
      checkpoint = checkpoint || 0.05;
      const e = (ease instanceof Ease) ? ease : (!ease) ? TweenLite.defaultEase : new Ease(ease);
      return Math.abs( (end - start) * e.getRatio(checkpoint) / velocity / checkpoint );
    };

    const _calculateTweenDuration = ThrowPropsPlugin.calculateTweenDuration = (target, vars, maxDuration, minDuration, overshootTolerance, recordEnd) => {
      ...
      let duration = 0;
      let clippedDuration = 9999999999;
      const throwPropsVars = vars.throwProps || vars;
      const ease = (vars.ease instanceof Ease) ? vars.ease : (!vars.ease) ? TweenLite.defaultEase : new Ease(vars.ease);
      const checkpoint = isNaN(throwPropsVars.checkpoint) ? 0.05 : Number(throwPropsVars.checkpoint);
      const resistance = isNaN(throwPropsVars.resistance) ? ThrowPropsPlugin.defaultResistance : Number(throwPropsVars.resistance);
      let p;
      let curProp;
      let curDuration;
      let curVelocity;
    ...
    const p = ThrowPropsPlugin.prototype = new TweenPlugin("throwProps");
    let _cssProxy;
    let _cssVars;
    let _last;
    let _lastValue;
    ...
    ThrowPropsPlugin.track = (target, props, types) => VelocityTracker.track(target, props, types);

    ThrowPropsPlugin.untrack = (target, props) => {
      VelocityTracker.untrack(target, props);
    };

    ThrowPropsPlugin.isTracking = (target, prop) => VelocityTracker.isTracking(target, prop);

    ThrowPropsPlugin.getVelocity = (target, prop) => {
      const vt = VelocityTracker.getByTarget(target);
      return vt ? vt.getVelocity(prop) : NaN;
    };

    ThrowPropsPlugin._cssRegister = () => {
      const CSSPlugin = _globals.com.greensock.plugins.CSSPlugin;
      if (!CSSPlugin) {
        return;
      }
      const _internals = CSSPlugin._internals;
      const _parseToProxy = _internals._parseToProxy;
      const _setPluginRatio = _internals._setPluginRatio;
      const CSSPropTween = _internals.CSSPropTween;
      _internals._registerComplexSpecialProp("throwProps", {parser(t, e, prop, cssp, pt, plugin) {
    ...
    ThrowPropsPlugin.to = (target, vars, maxDuration, minDuration, overshootTolerance) => {
      if (!vars.throwProps) {
      ...
      let tween = new TweenLite(target, minDuration || 1, vars);
    ...
    p._onInitTween = function(target, value, tween, index) {
      ...
      _last = this;
      _lastValue = value;
      const ease = tween._ease;
      const checkpoint = isNaN(value.checkpoint) ? 0.05 : Number(value.checkpoint);
      const duration = tween._duration;
      const preventOvershoot = value.preventOvershoot;
      let cnt = 0;
      let p;
      let curProp;
      let curVal;
      ...
      for (p in value) {
        if (p !== "resistance" && p !== "checkpoint" && p !== "preventOvershoot" && p !== "linkedProps" && p !== "radius") {
          ...
          this._props[cnt++] = {p, s:curVal, c1:change1, c2:change2, f:isFunc, r:false};
        }
      }
      return true;
    };
   ...
   * ----------------------------------------------------------------
   */
  _gsScope._gsDefine("utils.VelocityTracker", ["TweenLite"], TweenLite => {
    let _first;
    let _initted;
    let _time1;
    let _time2;
    ...
    const _empty = {};
    const _doc = _gsScope.document;
    const _transforms = {x:1,y:1,z:2,scale:1,scaleX:1,scaleY:1,rotation:1,rotationZ:1,rotationX:2,rotationY:2,skewX:1,skewY:1,xPercent:1,yPercent:1};
    const _getComputedStyle = _doc.defaultView ? _doc.defaultView.getComputedStyle : () => {};

    const _getStyle = (t, p, cs) => {
      let rv = (t._gsTransform || _empty)[p];
      if (rv || rv === 0) {
        return rv;
      } else if (t.style[p]) {
        rv = t.style[p];
    ...
    const _ticker = TweenLite.ticker;

    const VelocityProp = function(p, isFunc, next) {
      this.p = p;
      this.f = isFunc;
      this.v1 = this.v2 = 0;
    ...
    const VelocityTracker = function(target) {
      this._lookup = {};
      this.target = target;
    ...
    const getByTarget = VelocityTracker.getByTarget = target => {
      let vt = _first;
      while (vt) {
        if (vt.target === target) {
          return vt;
    ...
    const p = VelocityTracker.prototype;

    p.addProp = function(prop, type) {
      if (!this._lookup[prop]) {
        const t = this.target;
        const isFunc = (typeof(t[prop]) === "function");
        const alt = isFunc ? this._altProp(prop) : prop;
        let vp = this._firstVP;
    ...
    p._altProp = function(p) {
      ...
      const pre = p.substr(0, 3);

      const alt = ((pre === "get") ? "set" : (pre === "set") ? "get" : pre) + p.substr(3);
      return (typeof(this.target[alt]) === "function") ? alt : p;
    };

    VelocityTracker.getByTarget = target => {
      let vt = _first;
      if (typeof(target) === "string") {
        target = TweenLite.selector(target);
      }
    ...
    VelocityTracker.track = (target, props, types) => {
      let vt = getByTarget(target);
      const a = props.split(",");
      let i = a.length;
      types = (types || "").split(",");
      if (!vt) {
    ...
    VelocityTracker.isTracking = (target, prop) => {
      const vt = getByTarget(target);
...
((name => {
  const getGlobal = () => (_gsScope.GreenSockGlobals || _gsScope)[name];
